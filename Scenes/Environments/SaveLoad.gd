...
func clear_editor():
	DialogEditor.selected_responses = []
	DialogEditor.selected_nodes = []
	DialogEditor.node_index = 0
	var save_nodes = get_tree().get_nodes_in_group("Save")
...
func save_category():
	...
	if current_category_path!= null :
		var save_category = File.new()
		var save_nodes = get_tree().get_nodes_in_group("Save")
		save_category.open(current_category_path,File.WRITE)
...
func load_category(category_name):
	clear_editor()
	current_category_path = current_directory+"/dialogs/"+category_name+"/"+category_name+".ydec"
	var save_category = File.new()
	var loaded_dialogs = []
	...
	while(save_category.get_position() < save_category.get_len()):
		...
		var currently_loaded_dialog = DialogEditor.add_dialog_node(new_object.offset,new_object.dialog_title,new_object.node_index)
	...
	for i in loaded_responses:
		if i.to_dialog_id > 0:
			DialogEditor.connect_nodes(i.name,0,loaded_dialogs[i.to_dialog_id].name,0)
	save_category.close()
