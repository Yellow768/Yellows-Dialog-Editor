...
func get_faction_data(directory : String) -> Dictionary:
...
func is_valid_faction_data(file : FileAccess) -> bool:
...
func create_faction_dict_from_dat_file(file : FileAccess) -> Dictionary:
...
func find_faction_name_from_bytes(bytes : PackedByteArray,starting_pos : int) -> String:
	...
	while name_cursor_position < bytes.size():
		...
		if(bytes.slice(n_name,n_name+4)) != name_bytes_identifier:
			...
			return fact_name
	return "Malformed Faction Name"
		


func convert_bytes_to_s32(bytes : PackedByteArray) -> int:
	var spb = StreamPeerBuffer.new()
	spb.big_endian = true
	spb.data_array = bytes
	return spb.get_32()
	
func convert_bytes_to_s16(bytes: PackedByteArray) -> int:
	var spb = StreamPeerBuffer.new()
	spb.big_endian = true
	spb.data_array = bytes
	return spb.get_16()
