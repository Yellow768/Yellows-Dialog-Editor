...
var quest_dict := {}
var indexed_quest_categories : Array[String]= []
...
func index_quest_categories() -> Dictionary:
...
func index_quest_names(quest_category: String) -> Dictionary:
...
func return_quest_title_key(file : FileAccess) -> String:
	while(file.get_position() < file.get_length()):
		var current_line := file.get_line()
		if '"Title"' in current_line:
			current_line = current_line.erase(0,14)
			current_line = current_line.erase(current_line.length()-2,2)
			return current_line
	return ""
